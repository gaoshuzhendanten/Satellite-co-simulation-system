...
def import_scenario(scn_filename):
    ...
    for line in import_lines:
        ...
        # import bodies
        if line[0] == "B":
            line[5] = eval(line[5])  #body_color([r,g,b])
            line[6] = eval(line[6])  #body_position([m,m,m])
            line[7] = eval(line[7])  #body_velocity([m/s,m/s,m/s])

            orient_nums = re.findall(r"[-+]?\d*\.\d+|\d+", line[8])  #轨道方向
        ...
        # import vessels
            ...
            line[3] = eval(line[3])  #vessel_color
            line[4] = eval(line[4])  #vessel_position([m,m,m])
            line[5] = eval(line[5])  #vessel_velocity([m/s,m/s,m/s])
...
def main(scn_filename=None, start_time=0):
    ...
    # creating a window
    window = glfw.create_window(int(window_x), int(window_y), "卫星仿真系统", None, None)
...
def pick_scenario():
    ...
    scenarios_list += glob.glob("my_scenarios/*.osf")
